...
if errorlevel 1 (
  ...
  if /I "!AUTO_MODE!"=="1" (
    ...
    echo Git repository initialized automatically.
  ) else (
...
if not defined REMOTE_URL (
  ...
  if /I "!AUTO_MODE!"=="1" (
    echo Remote setup skipped in non-interactive mode.
  ) else (
...
endlocal & set "PROJECT_DIR=%TARGET_DIR%" & set "CURRENT_BRANCH=%CURRENT_BRANCH%" & set "LAST_COMMIT=%LAST_COMMIT%" & set "AUTO_MODE=%AUTO_MODE%"
