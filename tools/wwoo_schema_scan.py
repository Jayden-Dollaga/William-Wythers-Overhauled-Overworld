#!/usr/bin/env python3
"""
WWOO Schema Scanner — 26.1.2 Migration Diff Tool
====================================================
...
Usage:
    python3 wwoo_schema_scan.py [--root .] [--out scan_report]
...
REMOVED_COLUMN_KEYS = {"exclusion_radius_xz", "exclusion_radius_y", "required_empty_blocks"}
REMOVED_FOLIAGE_KEYS = {
    "extra_branch_steps", "extra_branch_length",
    "place_branch_per_log_probability", "can_grow_through",
}
MISC_REMOVED_KEYS = {
    "heightmap", "dusted", "predicate", "snowy", "creaking", "sapling_provider",
}
...
# Keys that, if found at top level of configured_feature/placed_feature, are required
...
# Mod namespaces to always skip (not part of this datapack's responsibility)
SKIP_NAMESPACES = (
...
def find_feature_id_misuse(obj, path=""):
    """
    Detect minecraft:patch_* (and similar) feature IDs used in WRONG contexts
    (block tags, blocks lists, predicates, blockstate Name fields) instead of
    their valid context (a "feature"/"features" key).
    """
    results = []
...
def find_missing_type(obj, file_path_str):
    """For configured_feature / placed_feature dirs, top-level 'type' must exist."""
...
def scan_file(path: Path, root: Path):
    """Returns a dict of issue_type -> list of detail dicts for this file."""
    ...
    if has_skip_namespace(raw_text) and "towns_and_towers" in rel:
        return None  # whole file skipped — compat tag file

    data, err = load_json_safe(path)
    if err:
        issues["json_parse_error"].append({"error": err})
        return issues  # can't do structural checks on unparsable JSON
    ...
    # 3. ColumnPlacer keys
    for jpath, key, val in find_keys_recursive(data, REMOVED_COLUMN_KEYS):
        issues["column_placer_keys"].append({"path": jpath, "key": key})

    # 4. FoliagePlacer keys
    for jpath, key, val in find_keys_recursive(data, REMOVED_FOLIAGE_KEYS):
...
def check_unbound_references(working_root: Path, vanilla_root: Path):
    """
    Scan all biome / placed_feature files for feature references
    (minecraft:xxx or wythers:xxx) and confirm the referenced
    configured_feature/placed_feature JSON actually exists somewhere
    in the working datapack OR in the vanilla 26.1.2 reference.
    """
    unbound = []

    # Build a set of all known feature ids that DO have a definition file
    known_ids = set()

    def register_known(root: Path, subdir: str, namespace_dir: str):
        base = root / "data"
        if not base.exists():
            return
    ...
    for subdir in ("configured_feature", "placed_feature"):
        register_known(working_root, subdir, subdir)
        if vanilla_root.exists():
            register_known(vanilla_root, subdir, subdir)

    # Now scan biome files + placed_feature "feature" refs for dangling pointers
    ...
    # Also check placed_feature -> configured_feature chains
...
# ----------------------------------------------------------------------
# Diff working vs WWOO_ORIGINAL — what's been touched / what's new
# ----------------------------------------------------------------------
    ...
    for sub in SCAN_SUBDIRS:
        ...
        if wbase.exists():
            for p in wbase.rglob("*.json"):
                working_files[relpath(p, working_root / "data") if False else relpath(p, wbase)] = p
        if obase.exists():
            for p in obase.rglob("*.json"):
                original_files[relpath(p, obase)] = p

    # Need consistent relative keying — redo using path from sub root, prefixed by sub
    working_files = {}
    original_files = {}
    for sub in SCAN_SUBDIRS:
        wbase = working_root / sub
...
def main():
    parser = argparse.ArgumentParser(description="WWOO 26.1.2 schema scanner")
    ...
    # ---- Per-file checklist scan ----
    print("Scanning all data/ files against breaking-change checklist...")
    all_issues = {}
    files_scanned = 0
    files_with_issues = 0
    ...
    for path in walk_json_files(working_root, SCAN_SUBDIRS):
        ...
        if result:
            ...
            all_issues[rel] = {k: v for k, v in result.items() if v}
            if all_issues[rel]:
                files_with_issues += 1
            else:
                del all_issues[rel]
    ...
    # ---- Aggregate counts by checklist category ----
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# WWOO 26.1.2 Schema Scan Report\n\n")
